...
func TestResolveLocalThimURI(t *testing.T) {
	testCases := []struct {
		name           string
		uri            string
		fabricNodeHost string
		expected       string
	}{
		{
			name:     "default endpoint",
			expected: defaultLocalThimURI,
		},
		{
			name:           "fabric node endpoint",
			fabricNodeHost: "10.92.0.24",
			expected:       "10.92.0.24:2377/metadata/THIM/amd/certification",
		},
		{
			name:           "fabric node IPv6 endpoint",
			fabricNodeHost: "2001:db8::1",
			expected:       "[2001:db8::1]:2377/metadata/THIM/amd/certification",
		},
		{
			name:           "explicit endpoint takes precedence",
			uri:            "thim.example.test/certification",
			fabricNodeHost: "10.92.0.24",
			expected:       "thim.example.test/certification",
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			t.Setenv(fabricNodeHostEnv, testCase.fabricNodeHost)

			actual := resolveLocalThimURI(testCase.uri)
