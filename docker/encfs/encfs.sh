...
cleanup_unmount() {
  mount_array=$(process_encoded_string $CmdlineArgs)
  echo "Cleaning up by unmounting the following remote filesystem folder: ${mount_array[@]}"
...
function process_encoded_string() {
    # Base64 encoded string
    local encoded_string="$1"
    echo "encoded string is "
    echo $encoded_string
    # Decode the string
    ...
    if echo "$decoded_string" | jq empty 2>/dev/null; then
        echo "The decoded string is a valid JSON object."
